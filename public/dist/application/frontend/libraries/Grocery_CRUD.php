		...
		foreach($this->basic_model->get_field_types_basic_table() as $field_info)
		{
			...
			if($this->change_field_type !== null && isset($this->change_field_type[$field_info->name]))
			{
				...
				elseif (isset($this->upload_fields[$field_info->name])) {
					...
					$field_info->extras2    = $field_type->extras2;
				}
...
//				echo $field_info->name . '-> ' . $real_type . '    ';
			}
		...
		switch ($real_type) {
			...
			case 'upload_file':
				...
				else
				{
					...
					if (!empty($this->alternate_file_dir))
						$file_url = $this->alternate_file_dir  . "/$value";
	...
	protected function get_total_results()
	{
		...
		if (!empty($this->inner_join))
			foreach ($this->inner_join as $item) {
				$this->basic_model->set_inner_join($item->Table, $item->Where);
			}
			...
			else
			{
				...
				foreach($columns as $column)
				{
					if(isset($temp_relation[$column->field_name]))
					{
						...
						{
							if ($first)
								$this->like($temp_relation[$column->field_name], $search_text);
							else
					...
					{
						if ($first)
							$this->like($column->field_name, $search_text);
						else
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected $default_true_false_text 	= array('غیرفعال' , 'فعال');
	...
	protected function showReadForm($state_info)
	{
		...
		if ($this->callback_read !== null) {
			call_user_func($this->callback_read, $data->field_values);
		}
	...
	protected function get_integer_input($field_info,$value)
	{
		...
		if (!isset($value) && isset($field_info->extras))
	...
	protected function get_multiline_input($field_info,$value)
	{
		$extra_attributes = '';
		if(!empty($field_info->db_max_length))
			$extra_attributes .= "maxlength='{$field_info->db_max_length}'";
...
/*		if (!empty($field_info->extras))
			$extra_attributes .= " style='width: 50%;'";*/

		$input = "<textarea id='field-{$field_info->name}' name='{$field_info->name}' class='form-control' $extra_attributes>$value</textarea>";

		if (!empty($field_info->extras)) {
			$data = $field_info->extras;

			$input .= '<div style="display: block; margin-right: 15px; " >';
			foreach ($data as $key => $item) {
				$input .= '<a href="javascript:" onclick="AddFieldToTextBox' . $field_info->name . "('{" . $key . "}');\">" . $item . '</a>&nbsp;|&nbsp;';
			}
			$input .= '</div>';

			$input .= "<script type='text/javascript'>
				function AddFieldToTextBox{$field_info->name} (key) {
					AddText('field-{$field_info->name}', key);
				}
			</script>
			";
		}
	...
	protected function get_map_input($field_info, $value) {
		$input = '<div id="map" style="width: 700px; height: 500px;"></div><input type="hidden" name="Map" id="field-Map" value="' . $value . '" />';
		$input .= '<script src="http://maps.googleapis.com/maps/api/js?v=3.exp&key=AIzaSyBTy2w9oSFIQj5hRu1yslL6WbPJV_uwvSA"></script>';
	...
	<script type="text/javascript">
		...
		function initialize() {
			try {
		    var latlng = new google.maps.LatLng(' . (empty($value) ? '36.27541722231454,59.6612548828125' : $value) . ');
		    ...
		    var markerIcon = { 
		      url: "' . base_url("assets/img/map_icon.png?3267752104786347736") . '", 
			...
			google.maps.event.addListener(map, \'click\', function(event) {
				try {
			   		...
			   		document.getElementById("field-Map").value = event.latLng.lat() + "," + event.latLng.lng();
				}
				catch (err) {
	...
	protected function get_enum_radio_input($field_info,$value)
	{
		$this->load_js_chosen();
...
//		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}' class='chosen-select chzn-rtl' data-placeholder='".$select_title."'>";
		...
		{
			$selected = !empty($value) && $value == $key ? "checked " : '';
			$input .= "<label><input id='field-{$field_info->name}-$key' class='radio-uniform'  type='radio' name='{$field_info->name}' value='$key' $selected /> ".$option."</label> ";
		}
...
//		$input .= "</select>";
	...
	protected function get_color_input($field_info,$value)
	{
		$this->set_js_lib($this->default_javascript_path.'/jscolor.js');
		$extra_attributes = '';
		if(!empty($field_info->db_max_length))
			$extra_attributes .= "maxlength='{$field_info->db_max_length}'";
		...
		if (!isset($value) && isset($field_info->extras))
			$value = $field_info->extras;
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$value' class='jscolor' $extra_attributes />";
		return $input;
	}
	...
	protected function get_text_input($field_info,$value)
	{
		...
		if (!empty($field_info->extras2)) {
			$data = $field_info->extras2;

			$input .= '<div style="display: block; max-width: 600px;" >';
			foreach ($data as $key => $item) {
				$input .= '<a href="javascript:" onclick="AddFieldToTextBox' . $field_info->name . "('{" . $key . "}');\">" . $item . '</a>&nbsp;|&nbsp;';
			}
			$input .= '</div>';
			...
			$input .= "<script type='text/javascript'>
				function AddFieldToTextBox{$field_info->name} (key) {
					try {
					CKEDITOR.instances['field-{$field_info->name}'].insertText(key);
					}
					catch (err) {
					}
					AddText('field-{$field_info->name}', key);
	...
	protected function get_datetime_input($field_info,$value)
	{
		...
		if($this->language !== 'english' && false)
		{
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$datetime' maxlength='19' class='datetime-input' style='min-width: 300px !important;' />
		<table width='auto' style='direction: ltr;'><tr><td><a class='datetime-input-clear' tabindex='-1'>".$this->l('form_button_clear')."</a></td>
		<td><span style='direction: ltr;'>({$this->ui_date_format}) hh:mm:ss</span></td></tr></table>";
		return $input;
	}

	protected function get_html_input($field_info,$value)
	{
		$input = "<input id='field-{$field_info->name}' type='hidden' name='{$field_info->name}' value='$value' />";
		$input .= $field_info->extras;
		
		return $input;
	}

	protected function get_access_input($field_info,$value)
	{
		$input = "<input id='field-{$field_info->name}' type='hidden' name='{$field_info->name}' value='" . decrypt($value) . "' />";
		$input .= $field_info->extras;
		
		return $input;
	}

	protected function get_shamsi_date_input($field_info,$value)
	{
		if (empty($value) && !empty($field_info->extras))
			$value = $field_info->extras;
		$input = "<input id='field-{$field_info->name}' type='text' name='{$field_info->name}' value='$value' />";
		$input .= "<script type='text/javascript'>
			try {
			var obj_{$field_info->name} = new AMIB.persianCalendar('field-{$field_info->name}', { extraInputID: 'field-{$field_info->name}', extraInputFormat: 'yyyy/mm/dd' });
			}
			catch (err) {
				console.log(err);
			}
	...
	protected function get_username_input($field_info,$value)
	{
		$value = !is_string($value) ? '' : $value;
		...
		if ($value != '')
			$extra_attributes .= ' readonly class="disabled"';

		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$value' $extra_attributes />";
		return $input;
	}

	protected function get_custom_password_input($field_info,$value)
	{
		$value = !is_string($value) ? '' : $value;
		$insert = true;
		$CI =& get_instance();
        for ($i = 0; $i < $CI->uri->total_segments() + 1; $i++) {
            $seg = $CI->uri->segment($i);
	...
	protected function get_multiselect_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}[]' multiple='multiple' size='8' class='chosen-multiple-select chzn-rtl' data-placeholder='$select_title' style='width:510px;' >";
	...
	protected function get_relation_input($field_info,$value)
	{
		...
		$ajax_or_not_class = $using_ajax ? 'chosen-select chzn-rtl' : 'chosen-select chzn-rtl';
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$input = '<label class="fileinput-button qq-upload-button" for="' . $unique . '" id="upload-button-'.$unique.'" style="'.$uploader_display_none.'">
...
class Grocery_CRUD extends grocery_CRUD_States
{
	 ...
	 */
	public function change_field_type($field , $type, $extras = null, $extras2 = null)
	{
		$field_type = (object)array('type' => $type);

		$field_type->extras = $extras;
		$field_type->extras2 = $extras2;
	 ...
	 * @param array|string $extras
	 */
	public function field_type($field , $type, $extras = null, $extras2 = null)
	{
		return $this->change_field_type($field , $type, $extras, $extras2);
	}
...
//@todo ui_day, ui_month, ui_year has to be lang strings
	 ...
	 * Enter description here ...
	 */
	public function callback_read($callback = null)
	{
		$this->callback_read = $callback;
	...
	public function set_inner_join($table, $where) {
		$this->inner_join[count($this->inner_join)] = (object)array(
			'Table'  => $table,
			'Where'  => $where
			);
	}
