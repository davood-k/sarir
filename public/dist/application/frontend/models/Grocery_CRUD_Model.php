...
class grocery_CRUD_Model  extends CI_Model  {
	...
	function set_inner_join($table, $where) {
        $unique_name = $this->_unique_join_name($table);
        $sql = '';
        foreach ($where as $item) {
            if ($sql != '')
                $sql .= " AND ";

            $sql .= "`{$item['lTable']}.{$item['lField']} = {$item['rTable']}`.{$item['rField']}";
        }

        $this->db->join( $table , $sql ,'inner');
